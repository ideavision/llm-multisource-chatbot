...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("passist_id", "prompt_id"),
    )

    # Changes to passist first so chat_sessions can have the right passist
    # The empty passist will be overwritten on server startup
    op.add_column(
        "passist",
        sa.Column(
            "user_id",
    ...
    op.execute("UPDATE passist SET search_type = 'HYBRID'")
    op.alter_column("passist", "search_type", nullable=False)
    op.add_column(
        "passist",
        sa.Column("llm_relevance_filter", sa.Boolean(), nullable=True),
    )
    op.execute("UPDATE passist SET llm_relevance_filter = TRUE")
    op.alter_column("passist", "llm_relevance_filter", nullable=False)
    op.add_column(
        "passist",
        sa.Column("llm_filter_extraction", sa.Boolean(), nullable=True),
    )
    op.execute("UPDATE passist SET llm_filter_extraction = TRUE")
    op.alter_column("passist", "llm_filter_extraction", nullable=False)
    op.add_column(
        "passist",
        sa.Column(
            "recency_bias",
    ...
    op.execute("UPDATE passist SET recency_bias = 'BASE_DECAY'")
    op.alter_column("passist", "recency_bias", nullable=False)
    op.alter_column("passist", "description", existing_type=sa.VARCHAR(), nullable=True)
    op.execute("UPDATE passist SET description = ''")
    op.alter_column("passist", "description", nullable=False)
    op.create_foreign_key("passist__user_fk", "passist", "user", ["user_id"], ["id"])
    op.drop_column("passist", "datetime_aware")
    op.drop_column("passist", "tools")
    op.drop_column("passist", "hint_text")
    op.drop_column("passist", "apply_llm_relevance_filter")
    op.drop_column("passist", "retrieval_enabled")
    op.drop_column("passist", "system_text")

    # Need to create a passist row so fk can work
    result = bind.execute(sa.text("SELECT 1 FROM passist WHERE id = 0"))
        ...
        op.execute(
            sa.text(
                ...
                INSERT INTO passist (
                    ...
                    llm_model_version_override, default_passist, deleted
                ) VALUES (
                    0, NULL, '', '', 'HYBRID', NULL,
    ...
    delete_statement = sa.text(
        """
        DELETE FROM passist
        WHERE name = 'Payserai' AND default_passist = TRUE AND id != 0
        """
    )
    ...
    op.execute("UPDATE chat_session SET passist_id = 0")
    op.alter_column("chat_session", "passist_id", nullable=False)
    op.add_column(
...
def downgrade() -> None:
    ...
    op.drop_constraint("passist__user_fk", "passist", type_="foreignkey")
    ...
    op.alter_column("passist", "description", existing_type=sa.VARCHAR(), nullable=True)
    op.drop_column("passist", "recency_bias")
    op.drop_column("passist", "llm_filter_extraction")
    op.drop_column("passist", "llm_relevance_filter")
    op.drop_column("passist", "search_type")
    op.drop_column("passist", "user_id")
    op.add_column(
        "document_retrieval_feedback",
    ...
    op.alter_column(
        "chat_session", "passist_id", existing_type=sa.INTEGER(), nullable=True
    )
    ...
    op.add_column(
        "chat_message",
        sa.Column("passist_id", sa.INTEGER(), autoincrement=False, nullable=True),
    )
    op.add_column(
        "chat_message",
        sa.Column(
