...
# Have Payserai API server running to use this.
import json
...
def create_new_session() -> int:
    data = {"passist_id": 0}  # Global default Passist
...
def run_chat() -> None:
    try:
        ...
        print(
            "Looks like you haven't started the Payserai Backend server, please run the FastAPI server"
        )
        exit()
        return
