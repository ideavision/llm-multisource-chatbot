...
from payserai.configs.app_configs import DOCUMENT_INDEX_NAME  # noqa: E402
from payserai.configs.constants import SOURCE_TYPE  # noqa: E402
...
class SelectionAnalysis:
    ...
    def do_request(self, query: str) -> dict:
        """Request the Payserai API

        Args:
            query (str): A query

        Returns:
            dict: The Payserai API response content
        """
        ...
        try:
            ...
            if response.status_code != 200:
                color_output(
                    (
                        "something goes wrong while requesting the Payserai API "
            ...
            color_output(
                f"Unable to request the Payserai API for the query '{query}': {e}",
                model="critical",
            )
            sys.exit(1)
    ...
    def extract_content(self, contents: dict) -> dict:
        """Extract the content returns by the Payserai API

        Args:
            contents (dict): The payserai response content
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Currently, to get this script working when the Payserai Auth is "
    ...
    parser.add_argument(
        ...
        default=3000,
        help=(
            "The Payserai Web (not the API) port. We use the UI to forward the requests to the API. "
            "It should be '3000' for local dev and '80' if Payserai runs using docker compose."
        ),
    )
    parser.add_argument(
        "-q" "--queries",
