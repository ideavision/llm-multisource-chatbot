...
from payserai.configs.constants import DEFAULT_BOOST
...
class User(SQLAlchemyBaseUserTableUUID, Base):
    ...
    passists: Mapped[List["Passist"]] = relationship("Passist", back_populates="user")
...
class Passist__DocumentSet(Base):
    __tablename__ = "passist__document_set"

    passist_id: Mapped[int] = mapped_column(ForeignKey("passist.id"), primary_key=True)
    document_set_id: Mapped[int] = mapped_column(
        ForeignKey("document_set.id"), primary_key=True
    )


class Passist__Prompt(Base):
    __tablename__ = "passist__prompt"
...
class Document(Base):
    ...
    # (as is passed around in Payserai)
...
class ChatSession(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    user_id: Mapped[UUID | None] = mapped_column(ForeignKey("user.id"), nullable=True)
    passist_id: Mapped[int] = mapped_column(ForeignKey("passist.id"))
    ...
    passist: Mapped["Passist"] = relationship("Passist")
...
class DocumentSet(Base):
    ...
    passists: Mapped[list["Passist"]] = relationship(
        "Passist",
        secondary=Passist__DocumentSet.__table__,
        back_populates="document_sets",
    )
...
class Prompt(Base):
    ...
    passists: Mapped[list["Passist"]] = relationship(
        "Passist",
        secondary=Passist__Prompt.__table__,
        back_populates="prompts",
    )


class Passist(Base):
    __tablename__ = "passist"
    ...
    # Allows the Passist to specify a different LLM version than is controlled
    ...
    # Default passists are configured via backend during deployment
    # Treated specially (cannot be user edited etc.)
    default_passist: Mapped[bool] = mapped_column(Boolean, default=False)
    # controls whether the passist is available to be selected by users
    is_visible: Mapped[bool] = mapped_column(Boolean, default=True)
    # controls the ordering of passists in the UI
    # higher priority passists are displayed first, ties are resolved by the ID,
    ...
    user: Mapped[User] = relationship("User", back_populates="passists")

    # Default passists loaded via yaml cannot have the same name
    __table_args__ = (
        Index(
            "_default_passist_name_idx",
            "name",
            unique=True,
            postgresql_where=(default_passist == True),  # noqa: E712
        ),
    )
