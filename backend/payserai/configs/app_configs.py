...
from payserai.configs.constants import DocumentIndexType
...
# restrict access to Payserai to only users with emails from those domains.
# E.g. `VALID_EMAIL_DOMAINS=example.com,example.org` will restrict Payserai
...
# DB Configs
#####
DOCUMENT_INDEX_NAME = "payserai_index"
...
VESPA_DEPLOYMENT_ZIP = (
    os.environ.get("VESPA_DEPLOYMENT_ZIP") or "/app/payserai/vespa-app.zip"
)
...
CONFLUENCE_CONNECTOR_LABELS_TO_INCLUDE = [
    ignored_tag
    for ignored_tag in os.environ.get("CONFLUENCE_CONNECTOR_LABELS_TO_INCLUDE", "").split(
        ","
    )
    if ignored_tag
]
...
#####
# If MODEL_SERVER_HOST is set, the NLP models required for Payserai are offloaded to the server via
