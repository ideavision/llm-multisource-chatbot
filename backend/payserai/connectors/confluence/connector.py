...
from payserai.configs.app_configs import CONFLUENCE_CONNECTOR_LABELS_TO_INCLUDE
...
from payserai.connectors.cross_connector_utils.html_utils import format_document_soup
...
from payserai.connectors.models import BasicExpertInfo
...
def _extract_confluence_keys_from_cloud_url(wiki_url: str) -> tuple[str, str]:
    """Sample
    https://paysera.atlassian.net/wiki/spaces/aml/overview
    wiki_base is https://paysera.atlassian.net/wiki
    space is 1234abcd
    """
...
def _extract_confluence_keys_from_datacenter_url(wiki_url: str) -> tuple[str, str]:
    """Sample
    https://intranet.paysera.net/confluence/display/aml/overview
    wiki_base is https://intranet.paysera.net/confluence
    space is 1234abcd
    """
...
class ConfluenceConnector(LoadConnector, PollConnector):
    def __init__(
        ...
        # pages.
        labels_to_include: list[str] = CONFLUENCE_CONNECTOR_LABELS_TO_INCLUDE,
    ) -> None:
        self.batch_size = batch_size
        self.continue_on_failure = continue_on_failure
        self.labels_to_include = set(labels_to_include)
    ...
    def load_credentials(self, credentials: dict[str, Any]) -> dict[str, Any] | None:
        ...
        # Construct the appropriate headers for basic authentication
        headers = {
            "Authorization": f"Basic {auth}"
        }

        if self.is_cloud:
            # For cloud, use the existing method, or modify as necessary for your use case.
            self.confluence_client = Confluence(
                url=self.wiki_base,
                username=self.username,
                password=credentials.get("confluence_access_token"),
                cloud=self.is_cloud,
            )
        else:
            # For self-hosted, use the headers with basic authentication
               self.confluence_client = Confluence(
                url=self.wiki_base,
                username=self.username,  # Basic auth username
                password=self.password_or_token  # Basic auth password
            )
        return None
    ...
    def _get_doc_batch(
        ...
        for page in batch:
            ...
            if time_filter is None or time_filter(last_modified):
                ...
                if self.labels_to_include:
                    page_labels = self._fetch_labels(self.confluence_client, page_id)
                    label_intersection = self.labels_to_include.intersection(page_labels)
                    if not label_intersection:
                        logger.info(
                            f"Page with ID '{page_id}' has a label which has been "
                            f"designated as allowed: {label_intersection}. Skipping."
                        )
                        continue
