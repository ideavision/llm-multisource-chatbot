...
from payserai.access.models import DocumentAccess
from payserai.configs.constants import PUBLIC_DOC_PAT
from payserai.db.document import get_acccess_info_for_documents
...
def get_access_for_documents(
    ...
    versioned_get_access_for_documents_fn = fetch_versioned_implementation(
        "payserai.access.access", "_get_access_for_documents"
    )
...
def get_acl_for_user(user: User | None, db_session: Session | None = None) -> set[str]:
    versioned_acl_for_user_fn = fetch_versioned_implementation(
        "payserai.access.access", "_get_acl_for_user"
    )
