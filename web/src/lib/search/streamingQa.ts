...
export const searchRequestStreamed = async ({
  ...
  let relevantDocuments: PayseraiDocument[] | null = null;
  try {
    ...
    const response = await fetch("/api/query/stream-answer-with-quote", {
      ...
      body: JSON.stringify({
        ...
        passist_id: passist.id,
        prompt_id: passist.id === 0 ? null : passist.prompts[0]?.id,
        retrieval_options: {
          run_search: "always",
          real_time: true,
          filters: filters,
    ...
    while (true) {
      ...
      completedChunks.forEach((chunk) => {
        ...
        if (Object.hasOwn(chunk, "top_documents")) {
          ...
          const topDocuments = chunk.top_documents as PayseraiDocument[] | null;
          if (topDocuments) {
