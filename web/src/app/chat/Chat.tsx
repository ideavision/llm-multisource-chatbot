...
import { AnswerPiecePacket, PayseraiDocument } from "@/lib/search/interfaces";
import {
  BackendChatSession,
  BackendMessage,
  DocumentsResponse,
...
import { ChatPassistSelector } from "./ChatPassistSelector";
...
import { PayseraiInitializingLoader } from "@/components/PayseraiInitializingLoader";
...
export const Chat = ({
  ...
  existingChatSessionPassistId: number | undefined;
  ...
  useEffect(() => {
    ...
    async function initialSessionFetch() {
      if (existingChatSessionId === null) {
        ...
        if (defaultSelectedPassistId !== undefined) {
          setSelectedPassist(
            availablePassists.find(
              (passist) => passist.id === defaultSelectedPassistId
            )
          );
        } else {
          setSelectedPassist(undefined);
        }
        setMessageHistory([]);
        return;
      }
      ...
      setSelectedPassist(
        availablePassists.find(
          (passist) => passist.id === chatSession.passist_id
        )
      );
  ...
  const { aiMessage } = selectedMessageForDocDisplay
    ...
    : { aiMessage: null };
  const [selectedDocuments, setSelectedDocuments] = useState<PayseraiDocument[]>(
    []
  );

  const [selectedPassist, setSelectedPassist] = useState<Passist | undefined>(
    existingChatSessionPassistId !== undefined
      ? availablePassists.find(
          (passist) => passist.id === existingChatSessionPassistId
        )
      : defaultSelectedPassistId !== undefined
      ? availablePassists.find(
          (passist) => passist.id === defaultSelectedPassistId
        )
      : undefined
  );
  const livePassist = selectedPassist || availablePassists[0];
  ...
  const onSubmit = async ({
    ...
    if (isNewSession) {
      currChatSessionId = await createChatSession(livePassist?.id || 0);
    } else {
    ...
    let retrievalType: RetrievalType =
      selectedDocuments.length > 0
        ...
        : RetrievalType.None;
    let documents: PayseraiDocument[] = selectedDocuments;
    ...
    try {
      ...
      for await (const packetBunch of sendMessage({
        ...
        promptId: selectedPassist?.prompts[0]?.id || 0,
              ...
              {livePassist && (
                <div className="sticky top-0 left-80 z-10 w-full bg-background/90">
                  <div className="ml-2 p-1 rounded mt-2 w-fit">
                    <ChatPassistSelector
                      passists={availablePassists}
                      selectedPassistId={livePassist.id}
                      onPassistChange={(passist) => {
                        if (passist) {
                          setSelectedPassist(passist);
                          router.push(`/chat?passistId=${passist.id}`);
                        }
                      }}
                    />
                  </div>
              ...
              {messageHistory.length === 0 &&
                ...
                !isStreaming && (
                  <ChatIntro
                    ...
                    selectedPassist={selectedPassist}
