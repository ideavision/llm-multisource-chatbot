...
import { buildFinalPrompt, createPassist, updatePassist } from "./lib";
...
export function PassistEditor({
  existingPassist,
  documentSets,
  llmOverrideOptions,
  defaultLLM,
}: {
  existingPassist?: Passist | null;
  ...
  const isUpdate = existingPassist !== undefined && existingPassist !== null;
  const existingPrompt = existingPassist?.prompts[0] ?? null;
  ...
  return (
    <div>
      ...
      <Formik
        ...
        initialValues={{
          name: existingPassist?.name ?? "",
          description: existingPassist?.description ?? "",
          system_prompt: existingPrompt?.system_prompt ?? "",
          task_prompt: existingPrompt?.task_prompt ?? "",
          disable_retrieval: (existingPassist?.num_chunks ?? 5) === 0,
          document_set_ids:
            existingPassist?.document_sets?.map(
              (documentSet) => documentSet.id
            ) ?? ([] as number[]),
          num_chunks: existingPassist?.num_chunks ?? null,
          llm_relevance_filter: existingPassist?.llm_relevance_filter ?? false,
          llm_model_version_override:
            existingPassist?.llm_model_version_override ?? null,
        }}
        validationSchema={Yup.object()
          .shape({
            name: Yup.string().required("Must give the Passist a name!"),
            description: Yup.string().required(
              "Must give the Passist a description!"
            ),
        ...
        onSubmit={async (values, formikHelpers) => {
          ...
          let passistResponse;
          if (isUpdate) {
            [promptResponse, passistResponse] = await updatePassist({
              id: existingPassist.id,
              existingPromptId: existingPrompt?.id,
              ...values,
              num_chunks: numChunks,
            });
          } else {
            [promptResponse, passistResponse] = await createPassist({
              ...values,
              num_chunks: numChunks,
            });
          }
          ...
          if (passistResponse && !passistResponse.ok) {
            error = await passistResponse.text();
          }

          if (error) {
            setPopup({
              type: "error",
              message: `Failed to create Passist - ${error}`,
            });
          ...
          <Form>
            <div className="pb-6">
              ...
              <TextFormField
                ...
                label="Name"
                disabled={isUpdate}
                subtext="Users will be able to select this Passist based on this name."
              />

              <TextFormField
                name="description"
                label="Description"
                subtext="Provide a short descriptions which gives users a hint as to what they should use this Passist for."
              />
              ...
              <TextFormField
                ...
                isTextArea={true}
                subtext={
                  'Give general info about what the Passist is about. For example, "You are an assistant for On-Call engineers. Your goal is to read the provided context documents and give recommendations as to how to resolve the issue."'
                }
                onChange={(e) => {
              ...
              <BooleanFormField
                ...
                subtext={`
                If set, the Passist will not fetch any context documents to aid in the response. 
                        ...
                        <div>
                          <SubLabel>
                            <>
                              ...
                              that this Passist should search through. If none
                              are specified, the Passist will search through all
              ...
              {llmOverrideOptions.length > 0 && defaultLLM && (
                <>
                  ...
                  <Text>
                    Pick which LLM to use for this Passist. If left as Default,
