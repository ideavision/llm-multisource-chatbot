...
export default async function Home() {
  // Disable caching so we always get the up to date connector / document set / passist info
  ...
  const passistResponse = results[4] as Response | null;
  ...
  if (passistResponse?.ok) {
    passists = await passistResponse.json();
  } else {
    console.log(`Failed to fetch passists - ${passistResponse?.status}`);
  }
